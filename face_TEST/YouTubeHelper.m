...
- (NSString *)authenticate {
    //Get auth object from keychain if available
     NSString *email=[self storedAuth];
...
- (NSString *)uploadPrivateVideoWithTitle:(NSString *)title description:(NSString *)description commaSeperatedTags:(NSString *)tags andPath:(NSString *)path {
    
    if (![self isAuthValid]) {
        NSLog(@"YouTubeHelper: User not authenticated yet.");
      /*  UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"YouTubeHelper: User not authenticated yet." delegate:Google_AuthViewController.class cancelButtonTitle:@"Cancel" otherButtonTitles:nil];
    ...
    return [self prepareUploadVideo];
}
...
- (NSString *)storedAuth {
   ...
   return [_youTubeService.authorizer userEmail];
}
...
//    }
    ...
    return [self uploadVideoWithVideoObject:video
             resumeUploadLocationURL:nil];
}

- (NSString *)uploadVideoWithVideoObject:(GTLYouTubeVideo *)video
    ...
    if (fileHandle) {
        ...
        self.uploadFileTicket = [service executeQuery:query
                                completionHandler:^(GTLServiceTicket *ticket,
                                    ...
                                    if (error == nil) {
                                   [self setYID:uploadedVideo.identifier];
                                        NSLog(@"Video Uploaded : %@", uploadedVideo.identifier);
